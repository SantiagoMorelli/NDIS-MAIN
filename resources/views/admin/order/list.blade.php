...
<div class="page">
  ...
  <div class="page-inner">
    ...
    <header class="page-title-bar">
      ...
      <!-- title -->
      <h1 class="page-title">NDIA Orders</h1>
    ...
    { data: 'service_booking_id', 
      render:function(data, type, row, meta){
        if (row.service_booking_id) {
          data = row.service_booking_id;
        } else {
          data = '<span class="badge badge-lg badge-danger">Error</span>';
        }
        return data;
      }
    },
    {data: 'shipping_total', orderable: false},
    {data: 'gst_total', orderable: false},
...
function resubmitOrder(id) {
  if (confirm("Are you sure to resubmit order?") == true) {
    var id = id;
    $("#loadercontainer2").addClass("showloader");   
    // ajax
    $.ajax({
      type:"POST",
      url: "{{ route('resubmitOrder') }}",
      data: { id: id },
      dataType: 'json',
      success: function(res){
        $("#loadercontainer2").removeClass("showloader");
        var oTable = $('#orderlist-datatable').dataTable();
        oTable.fnDraw(false);
      }
    });
