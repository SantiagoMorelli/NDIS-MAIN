...
class NdisInternalApiRepository
{
    ...
    public function sendEmailToPlanManager($requestData)
    {
        ...
        if (isset($requestData['order_discount'])) {
            $details['order_discount'] = $requestData['order_discount'];
        }
        ...
        $data = [
            ...
            'product_total' => round($details['product_total'],2),
            'shipping_total' => round($details['shipping_total'],2),
            'order_discount' => round(abs($details['order_discount']),2),
            'total_ex_tax' => round($details['total_ex_tax'],2),
            'gst_total' => round($details['gst_total'],2),
            'order_total' => round($details['order_total'],2),
        ...
        {
            $cc       =  $requestData['customer_email']; // config('ndis.email.cc_user_email');
            $bcc      = config('ndis.email.cc_user_email');
        }
        else
        {
        ...
        PdfOne::AddPage();
        PdfOne::setFooterCallback(function ($pdf) {
        ...
        PdfOne::writeHTML($html_content, true, false, false, false, '');
        $fileatt = PdfOne::Output(storage_path() . '/invoice_' . $details['invoice_no'] . '.pdf', 'F');
        $attachment = storage_path() . '/invoice_' . $details['invoice_no'] . '.pdf';

        try {
            $send = $this->common->sendEmail($to, $from, $subject, $body, $attachment, $cc, $bcc, $details);
           // $send = $this->common->sendEmail($to, $from, $subject, $body, $attachment, config('ndis.email.cc_user_email'), $details);
    ...
    public function serviceBook($requestData, $participatePlanId, $orderId)
    {
        ...
        $orderItems = OrderItems::select('id as order_item_id', 'item_quantity', 'item_price','is_toggle','toggle_item_quantity','toggle_item_price', 'product_category', 'product_category_item')->where('orders_id', $orderId)->Orderby('id')->get()->toArray();
        ...
        for ($i = 0; $i < $maxTrial; $i++) {
            ...
            foreach ($orderItems as $key => $oValue) {
                ...
                if (array_key_exists($pcArray[$i], $itemKey)) {
                    $isToggle = $oValue['is_toggle'];
                    if ($itemKey[$pcArray[$i]]['is_toggle'] == 1) {
                        $isToggle = 1;
                    }
                    ...
                    $itemKey[$pcArray[$i]] =
                        [
                            ...
                            'is_toggle' => $isToggle,
                            'toggle_item_quantity' => $oValue['toggle_item_quantity'],
                            'toggle_item_price' => $oValue['toggle_item_price'],
                        ];
                } else {
                    $itemKey[$pcArray[$i]] =
                        [
            ...
            foreach ($itemKey as $key => $pcItemValue) {
                ...
                $itemQuantity = $pcItemValue['item_quantity'];
                if ($pcItemValue['is_toggle'] == 1) {
                    $perUnitPrice = 1;
                    $itemQuantity = round($totalUnitPrice,0);
                }
                ...
                $productItems[$j]['quantity']      = $itemQuantity;
