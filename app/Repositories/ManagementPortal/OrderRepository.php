...
class OrderRepository
{
   ...
   //extract data
   public function extract_data($type){
       if($type=='NDIS'){
    ...
    public function fetchAllOrders() {
        ...
        $ordersData = BcmOrder::select('bcm_order.order_number','bcm_order.order_date','bcm_order.order_status','bcm_order.customer_first_name','bcm_order.customer_last_name', 'bcm_order.order_total','bcm_order.shipping_total','bcm_order.gst_total','bcm_order.payment_option',
        ...
        , 'ndisOrders.ndis_plan_management_option as ndis_type' )                
    ...
    public function getAllOrders() {
        // $ordersData=$this->extract_data('NDIS')->get()->toArray();
        $ndis_service_booking_table = 'ndis_service_booking';
    ...
    public function getOrderProducts($id) {
        ...
        $ordersData = BcmOrder::select('order_number','order_status','oi.id as item_id','item_name','item_quantity','item_price','oi.product_category_item')
    ...
    public function getPlanManagedOrders() {
        // $ordersData=$this->extract_data('Plan')->get()->toArray();
        $plan_option = $this->common->encrypt_decrypt(config('ndis.plan_management_option.plan_managed'));
        $ordersData = Orders::select('orders.id as id','order_number','order_date','order_status','customer_first_name','customer_last_name', 'order_total','orders.response','shipping_total','gst_total')
                ->where('orders.ndis_plan_management_option',$plan_option)
                ->orderBy('orders.order_date','DESC')->get()->toArray();
    ...
    public function getOrderItems($id) {
        ...
        $ordersData = Orders::select('orders.id as order_id','order_number','order_status','ndis_participant_number','oi.id as item_id','item_name','item_quantity','item_price','is_toggle','toggle_item_quantity','toggle_item_price','np.id as payment_id','np.status','claim_number','oi.response','oi.product_category_item')
    ...
    public function editOrderItem($requestData) {
       ...
       if ($order) {
            ...
            if (isset($requestData['is_toggle']) && $requestData['is_toggle'] == 1) {
                $order->is_toggle               = $requestData['is_toggle'];
                $order->toggle_item_quantity    = $requestData['toggle_item_quantity'];
                $order->toggle_item_price       = $requestData['toggle_item_price'];
            } else {
                $order->is_toggle = 0;
            }
            if ($order->save()) {
